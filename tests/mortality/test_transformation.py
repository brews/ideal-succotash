...
def test_make_tas_20yrmean_annual_histogram(basic_segment_weights):
    ...
    """
    expected = xr.Dataset(
        {"histogram_tas": (["region", "year", "tas_bin"], np.zeros((1, 22, 170)))},
        coords={
            "region": np.array(["foobar"]),
            "year": np.arange(2000, 2022),
            "tas_bin": np.arange(-104.5, 65.5),
        },
    )
    expected["histogram_tas"].loc[
        {"region": "foobar", "tas_bin": 1.5, "year": 2010}
    ] = 365.0
    expected["histogram_tas"].loc[
        {"region": "foobar", "tas_bin": 1.5, "year": 2011}
    ] = 365.0
    expected["histogram_tas"].loc[
        {"region": "foobar", "tas_bin": 1.5, "year": 2012}
    ] = 346.79998779
    ...
    ds_in = xr.Dataset(
        {
            "tas": (
                ["lon", "lat", "time"],
                np.ones((1, 1, 7666), dtype=np.float32),
            )
        },
        coords={
            "lon": [1.0],
